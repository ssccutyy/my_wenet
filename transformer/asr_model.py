...
from wenet.transformer.contraloss import MyContraLoss
from wenet.transformer.contraloss import ContraLoss
...
class ContrastiveASRModel(torch.nn.Module):
    """CTC-attention hybrid Encoder-Decoder model"""
    def __init__(
        self,
        vocab_size: int,
        ...
        self.encoder = encoder
        #self.linear = torch.nn.Linear(encoder.output_size(), encoder.output_size())
        # self.linear = torch.nn.Sequential(
        #     torch.nn.Linear(encoder.output_size(), encoder.output_size()),
        #     torch.nn.ReLU(),
        #     torch.nn.Dropout(dropout_rate),
        # )
        self.contra_linear = torch.nn.Linear(encoder.output_size(), encoder.output_size())
        self.ctc = ctc
        self.criterion_contra = mycontra
    ...
    def forward(
        ...
        assert (speech.shape[0] == speech_lengths.shape[0] == text.shape[0] ==
                text_lengths.shape[0]), (speech.shape, speech_lengths.shape,
                                         text.shape, text_lengths.shape)
        # 1. Encoder
        encoder_out, encoder_mask, mask_id, encoder_feature = self.encoder(speech, speech_lengths)
        encoder_out_lens = encoder_mask.squeeze(1).sum(1)

        # 2a. contrastive branch
        target_feature = self.contra_linear(encoder_feature)
        if self.ctc_weight != 1.0:
            loss_contra = self.criterion_contra(encoder_out, target_feature, mask_id, phone)
        else:
            loss_contra = None

        # 2b. CTC branch
        #ctc_input = self.linear(encoder_out)
        if self.ctc_weight != 0.0:
            loss_ctc = self.ctc(encoder_out, encoder_out_lens, text,
                                text_lengths)
        else:
        ...
        if loss_ctc is None:
            loss = loss_contra
        elif loss_contra is None:
            loss = loss_ctc
        else:
            loss = self.ctc_weight * loss_ctc + (1 -
                                                 self.ctc_weight) * loss_contra
        return {"loss": loss, "loss_contra": loss_contra, "loss_ctc": loss_ctc}
    ...
    def _calc_contrastive_loss(
        ...
        #                                    self.ignore_id)
        #ys_in_lens = ys_pad_lens + 1
        #不是encoder-decoder的结构，不需要用sos和eos
        #contrastive loss不需要标签

        # reverse the seq, used for right to left decoder
        # r_ys_pad = reverse_pad_list(ys_pad, ys_pad_lens, float(self.ignore_id))
        # r_ys_in_pad, r_ys_out_pad = add_sos_eos(r_ys_pad, self.sos, self.eos,
        #                                         self.ignore_id)
        #不是encoder-decoder结构，不需要reverse

        # 1. Forward contrastive
        #decoder_out, r_decoder_out, _ = self.decoder(encoder_out, encoder_mask,
        #                                              ys_in_pad, ys_in_lens,
        #                                              r_ys_in_pad,
        #                                              self.reverse_weight)
        target_feature = self.linear(encoder_feature)

        # 2. Compute attention loss
        loss_contra = self.criterion_contra(context_feature, target_feature, mask_id, phone)
        #r_loss_att = torch.tensor(0.0)
        # if self.reverse_weight > 0.0:
        #     r_loss_att = self.criterion_att(r_decoder_out, r_ys_out_pad)
        # loss_att = loss_att * (
        #     1 - self.reverse_weight) + r_loss_att * self.reverse_weight
        # acc_contra = th_accuracy(
        #     decoder_out.view(-1, self.vocab_size),
        #     ys_out_pad,
        #     ignore_label=self.ignore_id,
        # )
    ...
    def _forward_encoder(
        ...
        if simulate_streaming and decoding_chunk_size > 0:
            ...
            encoder_out, encoder_mask, _, _ = self.encoder(
                speech,
                speech_lengths,
                decoding_chunk_size=decoding_chunk_size,
                num_decoding_left_chunks=num_decoding_left_chunks
