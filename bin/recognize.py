...
def main():
    ...
    with torch.no_grad(), open(args.result_file, 'w') as fout:
        for batch_idx, batch in enumerate(test_data_loader):
            keys, feats, phone, target, feats_lengths, target_lengths = batch
            ...
            for i, key in enumerate(keys):
                ...
                # idx_errors,_ = pycorrector.correct(content_str)
                # idx_list = list(idx_errors)
                logging.info('{} {}'.format(key, args.connect_symbol.join(content)))
                fout.write('{} {}\n'.format(key, args.connect_symbol.join(content)))
                # logging.info('{} {}'.format(key, args.connect_symbol.join(idx_list)))
                # fout.write('{} {}\n'.format(key, args.connect_symbol.join(idx_list)))
